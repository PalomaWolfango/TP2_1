...
export default class editLayerComponent extends Component {
        ...
        <div id="categories-menu">
          <ul>
            ...
            {this.state.categories.map((c) => (
              <li key={c.id}>
                <input type="checkbox" value={c.id}></input> {c.name}
              </li>
            ))}
          </ul>
        </div>
  ...
  _onFeatureGroupReady = async (reactFGref) => {
    ...
    let data = await getPolygonGeoJson().then((res) =>
      res.filter(
        (f) => f.category == "Escolas" && f.geometry.type == "LineString"
      )
    );
...
var getPolygonGeoJson = async () => {
  return [
    ...
    },
    {
      ...
      geometry: {
        type: "Point",
        coordinates: [-8.847716, 41.697903],
      },
    },
    {
      category: "Escolas",
